...
def generate_report():
    ...
    year = int(twse_date[:4])
    month = int(twse_date[4:6])
    day = int(twse_date[6:8])
    ...
    for ri, (name, sym) in enumerate(tw_stocks_info):
        ...
        if data and len(data) > 0:
            ...
            close_f = float(close)
            open_f = float(open_p)
            # 漲跌%是從開盤價計算（TWSE的漲跌價差/開盤價*100）
            try:
                change_val = float(change_str.replace('+', '').replace(',', ''))
                pct = (change_val / (close_f - change_val)) * 100 if close_f != change_val else 0
            except:
    ...
    output_path = f'/root/.openclaw/reports/daily/股市觀測站_{date_str}.docx'
